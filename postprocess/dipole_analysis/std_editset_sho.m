...
%
% Usage: 
%             >> [STUDY, ALLEEG] = std_editset_sho(STUDY, ALLEEG, key1, val1, ...);  
% Inputs:
...
%                 option).
%   < Sho addind options for depth in inbrain >
%   'depthThreshold' - has to be minus (e.g. -2). In that case it's inbrain
%                      XX mm, if positive, XX mm away from scalp.
%
% Outputs:
...
%          Sho Nakagome, University of Houston (Added depthThreshold
%          option)
...
% ===== Sho modification =====
depthThreshold = get_varargin(varargin,'depthThreshold',-2);
% ============================
                ...
                if rv ~= 1
                    if idat ~= 0
                        if strcmp(g.inbrain,'on')
                            ...
                            indleft = eeg_dipselect(ALLEEG(idat), rv*100,'inbrain',depthThreshold); 
                        else
